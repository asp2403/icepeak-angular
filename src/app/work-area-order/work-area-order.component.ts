...
import { ASSIGN, BpmData, COMPLETE_PROCESSING } from '../dto/bpm';
...
export class WorkAreaOrderComponent implements OnInit {
  ...
  get assignNextHidden() {
    let isHidden = this.bpmActions?.nextAction != ASSIGN || this.authService.userDetails.id == this.order?.idManager;
    return isHidden;
  }

  get assignPrevHidden() {
    let isHidden = this.bpmActions?.prevAction != ASSIGN || this.authService.userDetails.id == this.order?.idManager;
    return isHidden;
  }
